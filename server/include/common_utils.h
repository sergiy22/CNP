...
//шлях до файлу
char* getServerPortPath(const char* server_port_path, char*server,int& port, char*path, char portDelim=':',char pathDelim='/'){
...
char* extractFilePath(char* filePath, const char* fileName,char delim=PATH_DELIM){
...
char* extractFileName(char* fileName,const char* filePath,char delim=PATH_DELIM){
    if (!filePath || !fileName)
        return NULL;
